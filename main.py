...
class GraphApp:
    ...
    def __init__(self, master: tk.Tk) -> None:
        ...
        # ``expression_var`` is kept for backward compatibility with previous
        # versions that exposed the formula via a ``StringVar`` (e.g. external
        # automation/scripts may still reference it).  The actual widget now
        # uses a ``Text`` control to gain undo/clipboard behaviour, so we keep
        # the variable in sync manually.
        self.expression_var = tk.StringVar()
        self.expression_var.trace_add("write", self._on_expression_var_changed)
        ...
        # Internal flag to avoid feedback loops when synchronising the text
        # widget with ``expression_var``.
        self._suppress_expr_var_update = False
    ...
    def _build_interface(self) -> None:
        ...
        self.expr_entry.bind("<<Modified>>", self._on_expression_modified)
    ...
    def _on_expression_modified(self, _event) -> None:
        """Synchronise ``expression_var`` when the text widget changes."""

        if not self.expr_entry.edit_modified():
            return

        self.expr_entry.edit_modified(False)

        if self._suppress_expr_var_update:
            return

        self.expression_var.set(self._get_expression())

    def _on_expression_var_changed(self, *_args) -> None:
        """Update the text widget when ``expression_var`` changes externally."""

        if self._suppress_expr_var_update:
            return

        self._set_expression(self.expression_var.get(), update_var=False)
    ...
    def _set_expression(self, value: str, *, update_var: bool = True) -> None:
        """Update the expression input with *value* and reset undo history."""

        self._suppress_expr_var_update = True
        try:
